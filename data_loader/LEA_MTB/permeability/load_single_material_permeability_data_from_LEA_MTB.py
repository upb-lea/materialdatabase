...
def create_paths_to_LEA_MTB_measurements_from_config():
    ...
    path2measurement_data = Path(mdb.get_user_paths().external_material_data).joinpath(
        "LEA_MTB"
    )

    # Path to the directory of the processed measurement data
    path2processed_measurement_data = path2measurement_data.joinpath("permeability")
...
path2processed_measurement_data, path2probe_database = (
    create_paths_to_LEA_MTB_measurements_from_config()
)
...
path2mdb_data = (
    Path(mdb.get_user_paths().material_data)
    .joinpath("complex_permeability")
    .joinpath("LEA_MTB")
)
...
material_name = Material.N49.value
...
if not isdir(path2processed_material_data):
    ...
    directories_in_path2processed_data = [
        f
        for f in listdir(path2processed_material_data)
        if isdir(path2processed_material_data.joinpath(str(f)))
    ]
    logger.info(
        f"Found {len(directories_in_path2processed_data)} directories (probe names) in {path2processed_material_data} : "
    ...
    for directory in directories_in_path2processed_data:
        ...
        if directory in df_probe_database["probe_code"].values:
            ...
            logger.warning(
                f"The directory name {directory} does not match any probe name in the probe_database.csv \n"
        ...
        if len(files_in_path2processed_probe_data) != 1:
            ...
            df_probe = pd.read_csv(
                path2processed_probe_data.joinpath(
                    str(files_in_path2processed_probe_data[0])
                ),
                encoding="latin1",
            )
            ...
            logger.info(
                f"Write permeability data of material {material_name} measured on probe {probe_code} into the materialdatabase."
            )
        ...
        if df_material.empty:
            ...
            df_material.to_csv(
                path2mdb_data.joinpath(f"{material_name}.csv"), index=False
            )
