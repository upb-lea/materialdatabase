...
"""
  ...
  INSTRUCTIONS:
    ...
    1. CHANGE VARIABLE material TO DESIRED MATERIAL
    2. ENTER CORRECT PATH TO MagNet-Data 
    3. CHANGE ENUMERATION IN VARIABLE mu TO DESIRED MATERIAL (Material.XXX)
    4. CHANGE ENUMERATION IN VARIABLE datadict TO DESIRED MATERIAl (MagNetFileNames._XXX.value)
    5. RUN SCRIPT
    
"""

SINE = True  # SET TRUE TO RUN SINE DATA
TRIANGLE = False  # SET TRUE TO RUN TRIANGULAR DATA
TRAPEZOID = False  # SET TRUE TO RUN TRAPEZOIDAL DATA

PROCESS_DATA = False  # SET TRUE TO PROCESS DATA IF NOT DATA IS LOADED FROM GIVEN PATH

WRITE = False  # SET TO WRITE DATA INTO DATABASE

material = "N49"  # 1.
path = os.path.join(pt.my_MagNet_data_path, material)  # 2.

mu = constants.mu_0 * material_db.get_material_attribute(material_name=Material.N49, attribute="initial_permeability")  # 3.

data_dict = mat73.loadmat(os.path.join(path, MagNetFileNames._N49.value))  # 4.
...
if SINE:
    ...
    if WRITE:
        create_permeability_measurement_in_database(material, "MagNet", company="Princeton", date=date, test_setup_name="MagNet", toroid_dimensions=probe,
                                                    measurement_method="tba", equipment_names="tba")
    ...
    for temperature in unique_temperature:
        for H_DC in unique_H_DC_offset:
            ...
            for frequency in unique_frequency:
                print(df_sine.query(filter_string).shape[0])
                if df_sine.query(filter_string).shape[0] > min_number_of_measurements:
                    ...
                    print("H_DC: ", H_DC)
                    # print("B_DC: ", df_sine.query(filter_string).sort_values('mag_flux_density')["B_DC"])
                    ...
                    b_ref, mu_r, mu_phi_deg = process_permeability_data(b_ref, mu_r, mu_phi_deg, smooth_data=True, crop_data=True, plot_data=True,
                                                                        f=frequency, b_min=0.005, b_max=0.3)
                    ...
                    if WRITE:
                        write_permeability_data_into_database(current_shape="sine", frequency=frequency, temperature=temperature, H_DC_offset=H_DC, b_ref=b_ref,
                                                              mu_r_abs=mu_r, mu_phi_deg=mu_phi_deg, material_name="N49", measurement_setup="MagNet")
...
if TRIANGLE:
    if PROCESS_DATA:
        ...
        permeability_amplitude = [calc_mu_r_from_b_and_h_array(b=b, h=h) for b, h in zip(mag_flux_density_offset, mag_field_strength_offset)]
        permeability_angle = [mu_phi_deg__from_mu_r_and_p_hyst(frequency=f, b_peak=abs(max(b, key=abs)), mu_r=mu, p_hyst=p)
                              for f, b, mu, p in zip(frequency, mag_flux_density_offset, permeability_amplitude, powerloss)]
...
# TRAPEZOID ----------------------------------------------------------------------------------------------------------------------------------------------------
if TRAPEZOID:  # TODO IMPLEMENT TRAPEZOID
