...
core_name = "R_24.6x20.25x20.5"  # d_out * d_in * h
core_dimensions = core_name[2:].split(sep="x")
material_name = "N87_new"
...
temperature_db = 25
material_name_db = "N87"
measurement_setup_db = "small_signal_auto"

measurements_path = os.path.join(my_wayne_kerr_measurements_path, "small_signal", core_name, material_name)

links = [os.path.join(measurements_path, "lk.csv"),
         os.path.join(measurements_path, "l11.csv"),
         os.path.join(measurements_path, "l22.csv")]
...
mu_r_complex_m = (2 * np.pi * f * h / (2 * np.pi) * np.log(d_outer / d_inner))**(-1) * np.array(Z_m_complex.imag + j * Z_m_complex.real) / constants.mu_0

# Alternative magnetization impedance
Z_m_complex_alt = Z_11_complex - Z_k * (np.array(np.cos(np.deg2rad(phi_k)) + j * np.sin(np.deg2rad(phi_k))))
...
if write_data:
    ...
    for i, f in enumerate(f[indices]):
        ...
        write_permeability_data_into_database(frequency=f,
                                              ...
                                              material_name=material_name_db,
                                              measurement_setup=measurement_setup_db,
